...
public class ImageTableLayout extends LinearLayout {
    ...
    private Drawable[][] _drawableMatrix;
    private int[][] _colorGrid;

    public ImageTableLayout(Context context) {
        super(context);
        _drawableMatrix = new Drawable[3][8];
        ...
        for (int y = 0; y < layers.length; y++) {
            ...
            for (int x = 0; x < layer.getChildCount(); x++) {
                ...
                if (child instanceof ImageView) {
                    ...
                    Log.d(TAG, "x" + x + " y" + y + " " + _colorGrid[y][x]);
                    imageView.setBackgroundColor(_colorGrid[y][x]);
